...
const AddAmbulance = () => {
    ...
    const onSubmit = (data) => {
        axios.post("http://localhost:5000/api/v1/ambulance", data)
            .then(response => {
                toast.success("Ambulance successfully Added");
            })
            .catch(error => {
    ...
    return (
        <div className='w-full px-12'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="bg-gray-200 w-full p-12 rounded-xl">
                <div class="relative">
                ...
                <div className='text-center mt-5'>
                    <button type="submit" class="text-white bg-[#0E7490] hover:bg-[#0E7498]/90 focus:ring-4 focus:outline-none focus:ring-[#3b5998]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#3b5998]/55 mr-2 mb-2">
                        Add Ambulance
                    </button>
                </div>
            </form>
